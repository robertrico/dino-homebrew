...
- [x] BLOCK 4 — + registers + alu. **PASSING on bench 2026-08-02, stepped and free-run.** FIRST BLOCK THAT COMPUTES THE SUM, and the block that caught the SA field reaching the '382s BIT-REVERSED — every ADD had been executing as AND since the ROMs were first burned. Hardware found and landed during the block: MDR0-7 and W0-7 were unwired. Rig faults, all the same family — a test asserting on something it could not observe: `block4.stepped` hard-coded one image's answer so a correct run of another printed FAIL; `block4.milestone` asserted END counts a free-run capture cannot see (the trigger is HALT falling = RESET asserting, the RC stretch then holds the machine 0.25-2.2s while the burst finishes in a few hundred us, so window and run never overlap — a CORRECT machine gave 5, 6 and 20); the HALT-rise wait was 2s and gave up on the first timeout, which is shorter than the RC stretch alone; and the step-clock guard reported success with Y1 running, because it asked "does CLK move?" when a live oscillator moves it for you. IRB comes off, OB0-7 goes on at U35.2/5/6/9/12/15/16/19 (the '373 zigzag — count chip pins). Remove the FLAG_Z strap: U49.5 drives U62.3 now. Test: exactly 6 END pulses then HALT forever, OB = 0x4D. bit-reverse(0x4D) = 0xB2 and nibble-swap(0x4D) = 0xD4, so a flipped or transposed ribbon self-names. U35 HAS NO RESET and a power-cycle does not clear it, so the PROGRAM poisons OB with 0xFF before computing: any run that starts destroys the old answer, and OB can only read 0x4D if this run reached the second OUT.
- [x] BLOCK 5 — **PASSING on bench 2026-08-02, at SW1=0xF7 and again at 0xFF.** + io. All ten boards, NOT single-stepped. OB0-7 moves to the io end (R9-R16 pin 1), same Mega pins. STRAP IS0-7 = SW1 at 0xF7: its only 0-bit is W3, exactly the bit of the answer, so a leaking '244 turns OB into 0x00 and names itself. No scope work here — save the probe budget.
...
- [x] INPUT — **PASSING on bench 2026-08-02.** `IN` at opcode 0x52, word 0x103A (`END | src=SW | dst=REG_B`). One microword, no hardware: `SRC` already encoded `SW=7`, and `U50` makes `LE_TMP_B = NOR(~{REG_B_LOAD}, CLK)` — the shadow latch follows the LOAD STROBE, not the `LDBI` opcode — so `dst=REG_B` fills `TMP_B` and `ADD` sees it with no extra row. Only U9 reburns (0xAD70 -> 0x0FBB); `IN`'s high byte 0x10 already matched the safe-fill at 0x520, so U15 is unchanged. `PROG_in` (crc 0x2B9C) takes the second addend off SW1: 0x2F + 0x01 = 0x30, 0x2F + 0x1E = 0x4D. THE DIP READS BACKWARDS FROM THE BUS — pull-ups to +5V and a grounded common mean a CLOSED switch reads 0, so the silkscreen says ON where the bus reads zero, and the POSITION NUMBERING is silkscreen rather than copper so it cannot be netlist-checked at all. See roms/README.md.

ALL TEN MODULES AND ALL FIVE BLOCKS BENCH-PROVEN as of 2026-08-02: root, pc, microcode, control_word, mdr, registers, mar, memory, alu, io; blocks 1-5. Coverage lint reports 0 gaps and 0 pending. Timing is RETIRED — blocks 4 and 5 free-run at 1.024MHz with Y1 seated and produce 0x4D, and "works single-stepped, fails free-run" never appeared. The milestone program (LDAI 0xFF; OUT; LDAI 0x2F; LDBI 0x1E; ADD; OUT; HALT) is burned and seated.

WHAT REMAINS is the roadmap, in Rico's stated order: minimum work to reach the 16550 UART so DINO can talk to a terminal, then a proto-Monitor, then the wider ISA. `IN` is the front half of I/O; `OUT` to a UART data register instead of the LEDs is the back half, and the I/O decode '138 is what it needs. Also still unrun though burned and policed: LDA/STA/JMP/JNZ and the other seven ALU codes — the coverage images `alu`, `mem`, `flow` and `loop` are generated and waiting, and `mem` is the only witness for MAR-as-a-latch.

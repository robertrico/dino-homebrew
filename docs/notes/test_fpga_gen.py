...
def test_sheets_constant_has_ten_entries():
    # CLAUDE.md/the brief: "Ten sheets" -- the root schematic plus the nine
    # sub-sheets it owns (dino_v0_0_2/*.kicad_sch, one per module).
    assert len(fpga_gen.SHEETS) == 10
    assert len(set(fpga_gen.SHEETS)) == 10, "SHEETS has a duplicate token"
...
def test_no_hand_edit_marker(tmp_path):
    ...
    assert len(files) == 10, f"expected 9 sheet entities + dino_core.vhd, got {len(files)}"
    for p in files:
